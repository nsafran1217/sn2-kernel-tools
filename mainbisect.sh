...
# Copy kernel config
cp ./config/sn2-4.19.ns.config ./src/linux/.config

# Build the kernel
cd $_BASE_DIR/src/linux-main-bisect
...
mkdir $_BASE_DIR/release/bisect-linux-${_localversion}-ia64
#move tar
mv "$tarname" "$_BASE_DIR/release/bisect-linux-${_localversion}-ia64/"

#cp vmlinuz
mkdir "$_BASE_DIR/release/bisect-linux-${_localversion}-ia64/boot"
cp vmlinux.gz "$_BASE_DIR/release/bisect-linux-${_localversion}-ia64/boot/vmlinuz-$ver"
#cd
cd "$_BASE_DIR/release/bisect-linux-${_localversion}-ia64/"
#extract then recombine
tar xf "$_BASE_DIR/release/bisect-linux-${_localversion}-ia64/$tarname"
rm -f "$_BASE_DIR/release/bisect-linux-${_localversion}-ia64/$tarname"
