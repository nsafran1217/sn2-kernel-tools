...
# Directories
BASE_DIR=$(pwd)
BUILD_DIR="${BASE_DIR}/linux"  # Branch defaults to 'main' if not passed
...
# Start build
cd "${BUILD_DIR}"
LAST_KERNEL_VERSION=$(cat "${BASE_DIR}/linux-last-kernel-version" 2>/dev/null || true)
...
if ${BUILD_COMMAND}; then
    ...
    echo "${KERNEL_VERSION}" > "${BASE_DIR}/linux-last-kernel-version"
else
