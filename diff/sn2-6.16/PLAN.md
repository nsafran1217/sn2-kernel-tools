...
- 120 files, 0 failures after fix
### v6.4 → v6.6
- 4 rejected hunks: Kconfig (KEXEC→ARCH_SUPPORTS_KEXEC), mspec.c
  (vm_flags_set), serial/Makefile reanchor
- 5 build/boot fixes: snsc.c class_create single-arg, setup.c
  Root_HDA1 removal, tiocx.c const uevent, sn_console.c platform
  device for serial core, ioc4_serial.c ctrl_id for rs422 ports
- 120 files, 0 failures after fixes
### v6.6 → v6.16
- NOTE: ia64 was removed in v6.7; 6.16 clean tree has ia64 re-added
- 3 rejected hunks in 2 files: drivers/char/Makefile (MSPEC line gone),
  drivers/tty/serial/Makefile (alphabetical reorder)
- 2 missing files: mspec.c and uncached.c removed with ia64 in 6.7,
  carried forward from 6.6 patched tree
- 4 build fixes: sn_console.c + ioc4_serial.c uart_circ→kfifo migration,
  tiocx.c const bus_type, setup.c module_alloc/module_memfree removed
  (replaced by execmem subsystem in 6.12, no arch override possible)
- Known issue: VM_FLUSH_RESET_PERMS can no longer be suppressed for
  module loading; may cause SHUB PTC.GA interference. Recommend
  building modular drivers as built-in.
- 120 files, 0 failures after fixes
